...
//reorg data to struct of arrays
struct volume {
    size_t size;
    size_t last;
    float* x;
...
void post_process(struct volume* v, float* cx, float* cy) {
    //below taken from https://software.intel.com/en-us/articles/using-an-intel-cilk-plus-reducer-in-c
    static CILK_C_REDUCER_OPADD(mass_sum,double,0);
    CILK_C_REGISTER_REDUCER(mass_sum);
    static CILK_C_REDUCER_OPADD(wx,double,0);
    CILK_C_REGISTER_REDUCER(wx);
    static CILK_C_REDUCER_OPADD(wy,double,0);
